<template>
    <div class="space-y-5">
        ...
        <div v-if="loading" class="text-center py-16 text-gray-400">
            <svg
                class="w-8 h-8 mx-auto mb-3 animate-spin text-blue-400"
                fill="none"
                viewBox="0 0 24 24"
            >
                <circle
        ...
        <div
            v-else-if="error"
            class="bg-red-50 border border-red-200 rounded-xl px-5 py-4 text-red-600 text-sm"
        >
            {{ error }}
            <button @click="fetchAll" class="ml-3 underline font-medium">
                Coba lagi
            </button>
        </div>
        ...
        <template v-else>
            ...
            <div class="grid grid-cols-4 gap-4">
                <div
                    ...
                    :key="s.label"
                    class="rounded-xl p-4 flex items-center gap-4 shadow-sm border"
                    :class="s.bg"
                >
                    <div class="text-3xl">{{ s.icon }}</div>
                    <div>
                        <p class="text-2xl font-bold" :class="s.text">
                            {{ s.count }}
                        </p>
                        <p class="text-xs font-semibold" :class="s.text">
                            {{ s.label }}
                        </p>
                    </div>
                </div>
            ...
            >
                ...
                <div class="flex items-center gap-3">
                    <p class="text-xs text-gray-400">
                        Menampilkan mobil yang tidak sedang disewa
                    </p>
                    <button
                        @click="fetchAll"
                    ...
                    <div class="p-4">
                        ...
                        <div class="mb-3">
                            ...
                            <p class="text-xs text-gray-600">
                                {{ car.car_condition || "Tidak ada catatan" }}
                            </p>
                        </div>
                        ...
                        <p
                            v-else
                            class="text-xs text-orange-600 font-medium text-center"
                        >
                            Sedang dalam perbaikan
                        </p>

                        <p class="text-[10px] text-gray-300 mt-2">
                            Diperbarui: {{ formatDate(car.updated_at) }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100">
                ...
                >
                    <h3 class="font-semibold text-gray-800">
                        🔧 Catatan Maintenance Aktif
                    </h3>
                    <span
                        class="text-xs bg-orange-100 text-orange-700 font-semibold rounded-full px-2 py-0.5"
                        >{{ activeRecords.length }} aktif</span
                    >
                </div>
                <div
                            ...
                            >
                                ...
                                >
                                    ...
                                    <p class="text-[10px] text-gray-400">
                                        {{ rec.car?.license_plate }}
                                    </p>
                                </td>
                                <td
                                    class="px-5 py-3 text-gray-600 text-xs max-w-[180px]"
                                >
                                    {{ rec.description }}
                                </td>
                                <td
                                    class="px-5 py-3 text-gray-600 text-xs whitespace-nowrap"
                                >
                                    {{ rec.workshop_name }}
                                </td>
                                <td
                                    class="px-5 py-3 text-right font-semibold text-gray-800 whitespace-nowrap"
                                >
                                    Rp {{ formatPrice(rec.repair_cost) }}
                                </td>
                                <td
                                    class="px-5 py-3 text-xs text-gray-600 whitespace-nowrap"
                                >
                                    {{ rec.assigned_staff?.name || "-" }}
                                </td>
                                ...
                                <td
                                    class="px-5 py-3 text-xs text-gray-500 max-w-[180px]"
                                >
                                    {{ rec.staff_notes || "-" }}
                                </td>
                                <td class="px-5 py-3 text-center">
        ...
        >
            ...
            >
                <!-- Header -->
                <div
                    class="px-6 py-4 border-b border-gray-100 flex items-center justify-between bg-orange-50 rounded-t-2xl"
                >
                    <div>
                ...
                <div v-if="wizardStep === 1" class="px-6 py-4 space-y-4">
                    <div class="bg-gray-50 rounded-xl p-4">
                        ...
                        <p class="font-bold text-gray-900">
                            {{ wizardCar?.brand }} {{ wizardCar?.model }}
                        </p>
                        <p class="text-xs text-gray-500">
                            {{ wizardCar?.license_plate }} ·
                            {{ wizardCar?.car_code }}
                        </p>
                    </div>
                    <div>
                ...
                <div v-if="wizardStep === 2" class="px-6 py-4 space-y-4">
                    <div>
                        ...
                        >
                        <input
                            v-model="wizardForm.workshop_name"
                            type="text"
                            placeholder="Contoh: Bengkel Jaya Motor"
                            class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500"
                        />
                        <p
                            v-if="wizardErrors.workshop_name"
                            class="text-red-500 text-xs mt-1"
                        >
                            {{ wizardErrors.workshop_name }}
                        </p>
                    </div>
                    <div>
                        <label
                        ...
                        />
                        <p
                            v-if="wizardErrors.repair_cost"
                            class="text-red-500 text-xs mt-1"
                        >
                            {{ wizardErrors.repair_cost }}
                        </p>
                    </div>
                </div>
                ...
                <div v-if="wizardStep === 3" class="px-6 py-4 space-y-4">
                    <div>
                        <label
                            class="block text-sm font-medium text-gray-700 mb-1"
                            >Tugaskan Staff</label
                        >
                        <select
                            v-model="wizardForm.assigned_staff_id"
                        ...
                        <p class="text-xs text-gray-400 mt-1">
                            Staff yang ditugaskan akan melihat tugas ini di
                            dashboard mereka dan melaporkan progress perbaikan.
                        </p>
                    </div>
...
export default {
    ...
    computed: {
        ...
        filtered() {
            ...
            return this.nonRentedCars.filter(
                (c) =>
                    !s ||
                    `${c.brand} ${c.model}`.toLowerCase().includes(s) ||
                    c.license_plate.toLowerCase().includes(s) ||
                    c.car_code.toLowerCase().includes(s),
            );
        },
        statusSummary() {
            const avail = this.cars.filter(
                (c) => c.car_status === "Available",
            ).length;
            const rented = this.cars.filter(
                (c) => c.car_status === "Rented",
            ).length;
            const maint = this.cars.filter(
                (c) => c.car_status === "Maintenance",
            ).length;
        ...
        selectedStaffName() {
            ...
            const s = this.staffList.find(
                (x) => x.id === this.wizardForm.assigned_staff_id,
            );
            return s ? s.name : "-";
        },
    },
    ...
    methods: {
        ...
        nextStep() {
            ...
            if (this.wizardStep === 2) {
                if (!this.wizardForm.workshop_name.trim()) {
                    this.wizardErrors.workshop_name =
        ...
        async submitMaintenance() {
            ...
            try {
                ...
                this.showToast(
                    "success",
                    `${this.wizardCar.brand} ${this.wizardCar.model} dikirim ke maintenance.`,
                );
                await this.fetchAll();
            } catch (err) {
                this.showToast(
                    ...
                    err.response?.data?.message ??
                        "Gagal membuat maintenance record.",
                );
            } finally {
        ...
        async completeMaintenance(rec) {
            ...
            try {
                ...
                this.showToast(
                    "success",
                    `Maintenance ${rec.car?.brand} ${rec.car?.model} selesai. Mobil kembali Available.`,
                );
                await this.fetchAll();
            } catch (err) {
                this.showToast(
                    "error",
                    err.response?.data?.message ??
                        "Gagal menyelesaikan maintenance.",
                );
            } finally {
