        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            >
                <div class="flex items-start justify-between mb-3">
                    ...
                    <span
                        v-if="loading"
                        class="text-xs text-gray-300 animate-pulse"
                        >Memuat...</span
                    >
                </div>
                <div>
                    ...
                    <p
                        v-if="stat.sub"
                        class="text-xs mt-1"
                        :class="stat.subCls || 'text-gray-400'"
                    >
                        {{ stat.sub }}
                    </p>
                </div>
            ...
            >
                ...
                <div v-if="loading" class="p-5 space-y-3">
                    <div
                        v-for="i in 5"
                        :key="i"
                        class="h-8 bg-gray-100 rounded-lg animate-pulse"
                    ></div>
                </div>

                <div
                    v-else-if="recentRentals.length === 0"
                        ...
                        <tbody>
                            ...
                            >
                                ...
                                <td class="px-5 py-3 text-center">
                                    <span
                                        v-if="r.paidAt"
                                        class="text-xs font-medium text-green-700 bg-green-50 px-2 py-0.5 rounded-full capitalize"
                                    >
                                        ✓ {{ r.paymentMethod }}
                                    </span>
                                    <span
                ...
                <div class="p-5 space-y-5 flex-1">
                    ...
                    <div v-if="loading" class="space-y-4">
                        <div v-for="i in 3" :key="i" class="space-y-1">
                            <div
                                class="h-4 bg-gray-100 rounded animate-pulse w-2/3"
                            ></div>
                            <div
                                class="h-2 bg-gray-100 rounded animate-pulse"
                            ></div>
                        </div>
                    </div>
                    <template v-else>
    ...
    methods: {
        async fetchDashboardData() {
            ...
            try {
                ...
                const totalRevenue = completedTxs.reduce(
                    (sum, tx) =>
                        sum + parseFloat(tx.grand_total || tx.total_price || 0),
                    0,
                );
                ...
                this.stats = [
                    {
                        ...
                        icon: "🚗",
                        bg: "bg-blue-50",
                        sub: `${cars.filter((c) => c.car_status === "Available").length} tersedia`,
                        subCls: "text-green-500",
                    },
                    {
                        label: "Sewa Aktif",
                        ...
                        icon: "📋",
                        bg: "bg-green-50",
                        sub: "mobil sedang disewa",
                        subCls: "text-blue-500",
                    },
                    {
                        label: "Pendapatan Hari Ini",
                        value: "Rp " + this.formatRevenue(s.revenue_today ?? 0),
                        icon: "💰",
                        bg: "bg-yellow-50",
                        sub: `${s.completed_today ?? 0} transaksi selesai`,
                        subCls: "text-yellow-600",
                    },
                    {
                        label: "Selesai Hari Ini",
                        value: String(s.completed_today ?? 0),
                        icon: "🏁",
                        bg: "bg-purple-50",
                    },
                ];
                ...
                this.stats2 = [
                    ...
                    {
                        label: "Belum Dibayar",
                        value: String(s.unpaid_active ?? 0),
                        icon: "💳",
                        bg: "bg-red-50",
                    },
                    {
                    ...
                    {
                        label: "Total Pendapatan",
                        value: "Rp " + this.formatRevenue(totalRevenue),
                        icon: "💎",
                        bg: "bg-emerald-50",
                    },
                ];
                ...
                this.recentRentals = txs.slice(0, 7).map((tx) => ({
                    ...
                    amount:
                        "Rp " +
                        this.formatPrice(tx.grand_total ?? tx.total_price),
                }));

                // Fleet status dari cars
                const groups = { Available: 0, Rented: 0, Maintenance: 0 };
                cars.forEach((c) => {
                    if (groups[c.car_status] !== undefined)
                        groups[c.car_status]++;
                });
                this.fleetStatus = [
                    {
